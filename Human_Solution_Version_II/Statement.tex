...
\begin{itemize}
    ...
    \item \textbf{New slots:}
    \begin{itemize}
        \item Next line: $S$ (integer), the number of new slots introduced this week.
        \item Next $S$ lines: each line has the form
        \[
        \texttt{slot\_id mentor\_id time\_block}
        \]
        where \texttt{slot\_id} is unique across the entire program.
    \end{itemize}

    \item \textbf{New requests:}
    \begin{itemize}
        \item Next line: $R$ (integer), the number of new requests introduced this week.
        \item Next $R$ lines: each request line has fields separated by spaces:
        \[
        \texttt{request\_id fellow\_id required\_topics\_list}
        \]
        \[
        \texttt{acceptable\_times\_list urgency preferred\_mentor}
        \]
        where:
        \begin{itemize}
            \item \texttt{required\_topics\_list}: comma-separated topics (no spaces). Use \texttt{""} for empty.
    ...
    \item \textbf{Slot cancellations:}
    \begin{itemize}
        \item Next line: $C$ (integer), the number of slots cancelled this week.
        \item Next line: $C$ integers, each a \texttt{slot\_id:time\_block} to be canceled at this time slot.
    \end{itemize}

    \item \textbf{Slot rescheduling:}
    \begin{itemize}
        \item Next line: $U$ (integer), the number of slots rescheduled this week.
        \item Next $U$ lines: each line has the form
        \[
        \texttt{slot\_id time\_block new\_time\_block}
        \]
        The slot keeps the same mentor and slot ID; only the time block changes.
    \end{itemize}

    \item \textbf{Request cancellations:}
    \begin{itemize}
        \item Next line: $Q$ (integer), the number of requests cancelled this week.
        \item Next line: $Q$ integers, each a \texttt{request\_id:time\_block} to be removed before matching.
    \end{itemize}
\end{itemize}
...
\noindent\textbf{Notes:}
\begin{itemize}
    \item A cancelled slot is no longer available for assignment in that week.
    \item A rescheduled slot replaces its previous time block for that week.
    \item A cancelled request is ignored in that week and should not be matched.
    \item If a slot is both rescheduled and cancelled in the same week, the input should not contain such a conflict.
    \item If a request is cancelled in the same week it was introduced, it is treated as withdrawn before matching.
\end{itemize}
